// app/api/dashboard/team/generate-report/route.ts
...
export async function POST(request: Request) {
  try {
    ...
    const tone = (searchParams.get("tone") || "balanced") as Tone;

    const include = {
      highlights: searchParams.get("include_highlights") !== "false",
      insights: searchParams.get("include_insights") !== "false",
      recommendations: searchParams.get("include_recommendations") !== "false",
      workload: searchParams.get("include_workload") !== "false",
      sources: searchParams.get("include_sources") !== "false",
    };

    const body = (await request
      .json()
      .catch(() => ({ notes: null }))) as RequestBody;
    const { notes } = body;
    ...
    if (weekPreset === "this") {
      ...
      endBound = addDays(nowMon, 6);
    } else if (weekPreset === "custom" && customStart && customEnd) {
    ...
    const { data: activities } = await supabaseAdmin
      ...
      .in("user_id", memberIds.length ? memberIds : [-1]) // guard against empty
    ...
    let wrQuery = supabaseAdmin
      ...
      .in("user_id", memberIds.length ? memberIds : [-1])
      .eq("published", true)
    ...
    const payload = {
      ...
      options: {
        ...
        include: {
          ...
          workload_distribution: include.workload,
    ...
    const url = `${base.replace(/\/$/, "")}/webhook/team-generate-report`;
    ...
    const upstream = await fetch(url, {
      ...
      // @ts-expect-error - duplex is a non-standard fetch option
      duplex: "half",
      body: JSON.stringify(payload),
    });

    if (!upstream.ok || !upstream.body) {
      const errTxt = await upstream.text().catch(() => upstream.statusText);
      return NextResponse.json(
        { error: `n8n error: ${errTxt}` },
        { status: 502 },
      );
    }
    ...
    // Pass through as-is (SSE or chunked text). Disable buffering for proxies.
    const headers = new Headers({
      "Cache-Control": "no-cache, no-transform",
      "X-Accel-Buffering": "no",
      "Content-Type": ctype.includes("text/event-stream")
